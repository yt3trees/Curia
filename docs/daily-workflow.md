...
`tasks.md` is the unified task file for each project. How it is populated depends on whether Asana integration is configured.

### Without Asana (Local Task Mode)

If no Asana token is set, you can manage tasks directly in the app:

- Dashboard Today Queue `[+]` button - opens a task creation form. Fill in the task name, project, and optional due date, then click Create.
- Today Queue `Done` button - marks the task as completed directly in `tasks.md` (moves the checkbox from `[ ]` to `[x]`).
- You can also edit `tasks.md` by hand in the Editor. The file uses standard Markdown checkboxes under `## In Progress` and `## Completed` sections.

### With Asana

If an Asana token is configured, `tasks.md` is generated by `Asana Sync`. The Today Queue `Done` button marks the task complete in Asana. The `[+]` button on the Dashboard opens the Asana task creation flow via Quick Capture.
